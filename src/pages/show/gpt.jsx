                  ...
                  <div style={{marginRight: '83px'}}>
                    ...
                    <div className='size_selection' style={{width: '350px'}}>
                      {sizeArray.map((size, index) => (
                        <div key={size.id} className={`size_option ${selectedSizeIndex === index ? 'selected_size' : ''}`} onClick={() => { setSelectedSizeIndex(index); const selectedSizeId = size.id; setDefaultSize(selectedSizeId) }} >
                          {size.name}
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="d-flex">
                      {colorArray[selectedSizeIndex]?.color.map((color, index) => (
                        <div key={index} className="color_border me-4" style={{borderColor: selectedColorIndex === index ? '#4D4D4D' : '#E6E6E6', cursor: 'pointer'}} onClick={() => { setSelectedColorIndex(index); const selectedColorId = color.id; setDefaultColor(selectedColorId)}}>
...
// Shu yerda modalDatani ichidagi Размер va Цвет o'zgarsa dataBeck ham o'zgarib ketyabdi. modalDatani aloxida qilib ber. Bir birga aralashib ketamsin