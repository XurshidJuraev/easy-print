      ...
      <div className="container">
        <div className="d-flex">
          ...
          <div style={{width: '1000px', marginLeft: '80px', marginBottom: '400px'}}>
            ...
            {/* <p className='footer_delivery_text'>{localStorage.getItem('selectedLanguage') === 'ru' ? 'К оплате принимаются банковские карты платежных систем UZCARD и HUMO. Чтобы оплатить заказ банковской картой выберите способ “Оплатить онлайн при помощи банковской карты” на этапе оформления заказа. Чтобы перейти к этапу оплаты заполните все поля отмеченные звездочкой и нажмите на кнопку “Оформить заказ”.  Оплата происходит на специальной странице банка. На этой странице вы увидите общую сумму платежа и сможете указать сведения вашей карты. Данные будут сообщены только на авторизационный сервер банка по защищенному каналу. EasyPrint.uz не имеет доступа к данным вашей карты.' : 'Toʻlov uchun UZCARD va HUMO toʻlov tizimlarining bank kartalari qabul qilinadi. Buyurtmani bank kartasi bilan to`lash uchun buyurtmani joylashtirishda "Bank kartasi yordamida onlayn to`lov" usulini tanlang. To`lov bosqichiga o`tish uchun yulduzcha bilan belgilangan barcha maydonlarni to`ldiring va "Buyurtma berish" tugmasini bosing. To`lov bankning maxsus sahifasida amalga oshiriladi. Ushbu sahifada siz to`lovning umumiy miqdorini ko`rasiz va kartangiz ma`lumotlarini kiritishingiz mumkin. Ma`lumotlar faqat xavfsiz kanal orqali bankning avtorizatsiya serveriga xabar qilinadi. EasyPrint.uz sizning kartangiz ma`lumotlariga kirish huquqiga ega emas.'}</p> */}
